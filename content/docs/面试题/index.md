...
首先，红黑树喝平衡二叉树的查找时间复杂度都是O(log n)，其次因为平衡二叉树需要严格平衡，在插入删除时可能需要进行更多的旋转操作，在高并发的情况下，红黑树性能更优，
...
使用本地事务表+消息队列+定时任务的最终一致性方案

在业务写 MySQL 的同一个事务中，同时写入一张 **message_sync 表**，记录变更数据和状态，确保 **DB 写成功就一定有同步记录**。

后续通过 **定时任务（xxl-job）扫描未消费或失败状态的数据**，发送到 MQ，由消费者异步写入 Elasticsearch。

在消费端对 ES 写入做了 **幂等控制和失败重试**，并将写入结果回写到同步表中。

对于 **MQ 消费失败、ES 写失败、任务中断等异常场景**，都会落回同步表，确保数据可以被再次扫描和补偿。

通过额外的 **定期对账任务**，对 MySQL 和 ES 的数据量和关键字段进行校验，发现异常时触发补偿同步。
